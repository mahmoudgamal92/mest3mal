...
Route::post('/user/login', function (Request $request) {
    /// validation 
    $credentials = $request->validate(['email' => 'email', 'password' => 'min:6']);

    $user = User::where(['email' => $request->email, 'type' => 'user', 'status' => 'active'])->first();
...
//register route
Route::post('/user/new_register', function (Request $request) {
    $request->validate([
