...
class AgentsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables($query)->setRowId('id')
            ...
            ->rawColumns(['checkbox','agent_types','edit', 'delete'])
            ;
    }
    ...
    {
        $query = User::query();
        $query->with('agent_types')->where('id','!=',1)->select('users.*');
        return $this->applyScopes($query);
    }


    public function byAgentTypes($array = []) {
        $this->byAgentTypes = $array;
        return $this;
    }
