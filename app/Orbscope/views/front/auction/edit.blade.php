...
@section('content')
    ...
    <div class="main-section bg-main post-add-bg">
        <div class="container container-post-add">
            <form method="post" action="{{url('user/update/auction/'.$edit->id)}}"  enctype="multipart/form-data">
                {{ csrf_field() }}

                <div class="custom-bg-pd mb-3 post-detail-wrap post-ads-content">
                    <h3 class="add-post-title">{{trans('orbscope.edit')}} {{trans('front.auction')}}</h3>
                    <div class="post-detail-fields row">
                        ...
                        <div class="form-group col-12 form-control-004">
                            ...
                            <input type="text" name="title" value="{{$edit->title}}" required class="form-control form-control-123" id="AdTitle" placeholder="{{trans('front.auction_title')}}">
                        </div> <!-- title /-->
                        <div class="form-group col-12 form-control-004">
                            <label for="addescription" class="field-heading">{{trans('orbscope.details')}} </label>
                            <textarea class="form-control form-control-123" required name="details" id="addescription" placeholder="{{trans('front.product_details')}}" rows="3">{{$edit->details}}</textarea>
                        </div> <!-- title /-->
                        ...
                        <div class="form-group col-lg-12 form-group-123">
                            ...
                            <div class="form-group categori d-flex">
                                <select  required  name="duration" class="form-control">
                                    ...
                                    @for($x=1;$x<8;$x++)
                                        <option {{$edit->duration==$x?'selected':''}} value="{{$x}}">{{$x}}</option>
                                    @endfor
                                </select>
                            </div>
                        </div> <!-- Price /-->
                            ...
                            <div class="form-group categori">
                                ...
                                <div class="form-group categories-sort">
                                    <select  required name="city_id" class="form-control w-100">
                                        @foreach(\App\Orbscope\Models\City::where('status','active')->get() as $c)
                                        <option {{$edit->city_id==$ca->id?'selected':''}} value="{{$c->id}}" selected>{{VarByLang($c->name,GetLanguage())}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group col-lg-4 form-group-123">
                            <div class="form-group categori">
                                ...
                                <input type="text" value="{{$edit->address}}" name="address" class="form-control form-control-123" id="AdTitle" placeholder="">
                            </div>
                        </div> <!-- address /-->
